...
*/
class TinhDk_RandomPicture_widget extends WP_Widget {
	//Cài đặt mặc định
	...
	function __construct(){
		parent::__construct(
			'photo_list',
			'Widget Photo' . __( '', 'TinhDk' ),
			array( 'classname' => 'random_picture','description' => __( 'Danh sách hình ảnh của blog', 'TinhDk' ) )
		);
		$this->default_instance = array(
			'title'         => __( 'Photo', 'TinhDk' ),
			'id_base' => 'random_picture'
		);
	}
	...
	function widget( $args, $instance ) {
		...
		if ( '' !== $username ) {
			...
			if ( is_wp_error( $media_array ) ) {
				...
				$ulclass = apply_filters( 'wpiw_list_class', 'photostream group-' . $size );
...
register_widget( 'TinhDk_RandomPicture_widget' );
